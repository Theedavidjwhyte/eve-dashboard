    ...
    <div className="space-y-4">
                ...
                {byAD.sort((a, b) => b.value - a.value).map(ad => (
                  <div key={ad.fullName} className="flex items-center gap-2">
                    ...
                    <div className="flex-1 bg-muted rounded-full h-2 overflow-hidden">
                      <div className="h-2 rounded-full bg-amber-500 transition-all" style={{ width: `${Math.min(100, (ad.value / (totalValue || 1)) * 100)}%` }} />
                    </div>
                    ...
                    {ad.highRisk > 0 && <span className="text-[10px] text-destructive font-semibold">{ad.highRisk} 🔴</span>}
                  </div>
                ))}
              </div>
            )}
      ...
      <Card>
        ...
        <CardContent className="pt-0">
          {slipped.length === 0 ? (
            <p className="text-xs text-muted-foreground text-center py-12">🎉 No slipped deals — all pipeline deals are on track</p>
          ) : (
                    ...
                    return (
                      <TableRow key={i} className={rs >= 3 ? "bg-destructive/5" : rs === 2 ? "bg-amber-500/5" : ""}>
                        ...
                        <TableCell className="text-right">
                          <span className={`inline-flex items-center justify-center w-6 h-6 rounded-full text-[11px] font-bold ${pushCount >= 3 ? "bg-destructive/20 text-destructive" : pushCount === 2 ? "bg-amber-500/20 text-amber-500" : "bg-muted text-muted-foreground"}`}>
                            {pushCount}
                          </span>
                        </TableCell>
                        <TableCell className="text-right text-xs tabular-nums">
                          {(d._stageDur ?? 0) > 0 ? <span className={(d._stageDur ?? 0) > 60 ? "text-destructive font-semibold" : "text-muted-foreground"}>{d._stageDur}d</span> : <span className="text-muted-foreground">—</span>}
                        </TableCell>
                        <TableCell><RagBadge score={rs} /></TableCell>
                        <TableCell className="text-xs text-muted-foreground max-w-[160px] truncate">{(d["Next Step"] ?? "—").replace(/^⚡ NSF \| ?/, "").substring(0, 60)}</TableCell>
                      </TableRow>
                    )
                  })}
                </TableBody>
