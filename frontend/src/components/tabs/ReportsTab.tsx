...
export function ReportsTab() {
  ...
  // ── EXEC FINANCE UPDATE EXPORT ──
  function exportExecFinance() {
    const { arrDeals, arrBaseData: arrBase } = useDashboardStore.getState()
    const today2 = new Date().toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })

    // OI section per AD
    let oiCSV = "OI PIPELINE SUMMARY\n"
    oiCSV += "AD,Won Value,Won Deals,Commit Pipe,Total Forecast,Open Pipeline,Budget,Gap,Attainment %\n"
    USERS.forEach((u) => {
      ...
      const budget = getADBudget(u, MONTHS, oiTargets)
      const gap = won - budget
      const att = budget > 0 ? Math.round(won / budget * 100) + "%" : "N/A"
      oiCSV += `"${u}",${won.toFixed(0)},${data.filter(r=>r.User===u&&r._stageSummary==="Won").length},${commit.toFixed(0)},${(won+commit).toFixed(0)},${pipe.toFixed(0)},${budget.toFixed(0)},${gap.toFixed(0)},${att}\n`
    })
    // Team total
    const teamWon = data.filter(r=>r._stageSummary==="Won").reduce((s,r)=>s+(r._val??0),0)
    const teamBudget = USERS.reduce((s,u)=>s+getADBudget(u,MONTHS,oiTargets),0)
    const teamCommit = data.filter(r=>r._stageSummary==="Pipe"&&r._commit==="Commit").reduce((s,r)=>s+(r._val??0),0)
    const teamPipe = data.filter(r=>r._stageSummary==="Pipe").reduce((s,r)=>s+(r._val??0),0)
    oiCSV += `"TEAM TOTAL",${teamWon.toFixed(0)},${data.filter(r=>r._stageSummary==="Won").length},${teamCommit.toFixed(0)},${(teamWon+teamCommit).toFixed(0)},${teamPipe.toFixed(0)},${teamBudget.toFixed(0)},${(teamWon-teamBudget).toFixed(0)},${teamBudget>0?Math.round(teamWon/teamBudget*100)+"%":"N/A"}\n`

    // ARR section per AD
    let arrCSV = "\nARR CLOSED WON SUMMARY\n"
    arrCSV += "AD,ARR Won,Deals,ARR Budget,Attainment %,Remaining\n"
    USERS.forEach((u) => {
      const first = u.split(" ")[0]
      const adDeals = arrDeals.filter(d => d.assignedAD === first || d.assignedAD === u)
      const won = adDeals.reduce((s,d)=>s+(d.totalAbc??0),0)
      const arrBudget = MONTHS.reduce((s,m)=>{
        const key = Object.keys(BUDGET_AD_MAP).find(k=>BUDGET_AD_MAP[k as keyof typeof BUDGET_AD_MAP]===u)
        return s+(arrTargets[m]?.[key as keyof typeof BUDGET_AD_MAP]??0)
      },0)
      const att = arrBudget > 0 ? Math.round(won/arrBudget*100)+"%" : "N/A"
      arrCSV += `"${u}",${won.toFixed(0)},${adDeals.length},${arrBudget.toFixed(0)},${att},${(arrBudget-won).toFixed(0)}\n`
    })
    const teamARR = arrDeals.reduce((s,d)=>s+(d.totalAbc??0),0)
    const teamARRBudget = MONTHS.reduce((s,m)=>s+BUDGET_AD_KEYS.reduce((ss,k)=>ss+(arrTargets[m]?.[k]??0),0),0)
    arrCSV += `"TEAM TOTAL",${teamARR.toFixed(0)},${arrDeals.length},${teamARRBudget.toFixed(0)},${teamARRBudget>0?Math.round(teamARR/teamARRBudget*100)+"%":"N/A"},${(teamARRBudget-teamARR).toFixed(0)}\n`

    // Monthly OI actuals vs budget
    let monthlyCSV = "\nMONTHLY OI ACTUALS vs BUDGET\n"
    monthlyCSV += "Month," + USERS.map(u=>`${u.split(" ")[0]} Actual,${u.split(" ")[0]} Budget,${u.split(" ")[0]} %`).join(",") + ",Team Actual,Team Budget,Team %\n"
    MONTHS.forEach(m => {
      let row = `"${m}"`
      let teamAct = 0, teamBud = 0
      USERS.forEach(u => {
        const act = data.filter(r=>r.User===u&&r._stageSummary==="Won"&&r._month===m).reduce((s,r)=>s+(r._val??0),0)
        const bud = getADBudget(u,[m],oiTargets)
        teamAct+=act; teamBud+=bud
        row += `,${act.toFixed(0)},${bud.toFixed(0)},${bud>0?Math.round(act/bud*100)+"%":"N/A"}`
      })
      row += `,${teamAct.toFixed(0)},${teamBud.toFixed(0)},${teamBud>0?Math.round(teamAct/teamBud*100)+"%":"N/A"}`
      monthlyCSV += row + "\n"
    })

    const fullCSV = `E.V.E EXEC FINANCE UPDATE — Generated ${today2}\n\n${oiCSV}${arrCSV}${monthlyCSV}`
    downloadCSV(`EVE_Exec_Finance_Update_${today()}.csv`, fullCSV)
  }
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          <ExportCard
            ...
            label="Exec Finance Update CSV"
            desc="OI pipeline summary + ARR closed won + monthly actuals vs budget — one file, three sections"
            onClick={exportExecFinance}
            badge="NEW"
          />
        </CardContent>
      </Card>
