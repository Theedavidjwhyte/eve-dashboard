...
export interface ForecastPostRow {
  id: string
  week_label: string
  month: string
  quarter: string
...
export async function fetchForecastPosts(): Promise<ForecastPostRow[] | null> {
  const db = await getClient()
  if (!db) return null
  const { data } = await db
    .from("eve_forecast_posts")
    .select("*")
    .order("generated_at", { ascending: false })
  return data ?? null
}

export async function upsertForecastPost(post: ForecastPostRow): Promise<void> {
  const db = await getClient()
  if (!db) return
  await db.from("eve_forecast_posts").upsert(post, { onConflict: "id" })
}

export async function deleteForecastPost(id: string): Promise<void> {
  const db = await getClient()
  if (!db) return
  await db.from("eve_forecast_posts").delete().eq("id", id)
}

// ── Deal Qualifications sync ──────────────────────────────────────────────────
...
export async function fetchQualifications(): Promise<QualifyRow[] | null> {
  const db = await getClient()
  if (!db) return null
  const { data } = await db.from("eve_deal_qualifications").select("*")
  return data ?? null
}

export async function upsertQualification(oppId: string, oppName: string, review: Record<string, unknown>): Promise<void> {
  const db = await getClient()
  if (!db) return
  await db.from("eve_deal_qualifications").upsert(
    { opportunity_id: oppId, opp_name: oppName, data: review, updated_at: new Date().toISOString() },
    { onConflict: "opportunity_id" }
  )
}

// ── Manual ARR Deals sync ─────────────────────────────────────────────────────

export async function pushManualARRDeal(deal: Record<string, unknown>): Promise<void> {
  const db = await getClient()
  if (!db) return
  await db.from("eve_manual_arr_deals").upsert(
    { opportunity_id: deal.opportunityId, data: deal, updated_at: new Date().toISOString() },
    { onConflict: "opportunity_id" }
  )
}

export async function deleteManualARRDeal(oppId: string): Promise<void> {
  const db = await getClient()
  if (!db) return
  await db.from("eve_manual_arr_deals").delete().eq("opportunity_id", oppId)
}

export async function fetchManualARRDeals(): Promise<Record<string, unknown>[] | null> {
  const db = await getClient()
  if (!db) return null
  const { data } = await db.from("eve_manual_arr_deals").select("data")
...
export async function pushARRDeals(
  deals: ARRDeal[],
  dupLog: ARRDupLog[],
  exemptLog: ARRDeal[],
  importDate: string
  ...
  await db.from("eve_arr_imports").upsert(
    { id: "latest", import_date: importDate, row_count: deals.length, updated_at: now },
    { onConflict: "id" }
  )

  // Upsert deals in batches
  const batch = 200
  const dealRows = deals.map((d) => ({ opportunity_id: d.opportunityId, data: d, updated_at: now }))
  for (let i = 0; i < dealRows.length; i += batch) {
    await db.from("eve_arr_deals").upsert(dealRows.slice(i, i + batch), { onConflict: "opportunity_id" })
  }

  // Upsert dup log
  const dupRows = dupLog.map((d) => ({ opportunity_id: d.opportunityId, data: d, updated_at: now }))
  if (dupRows.length > 0) {
    for (let i = 0; i < dupRows.length; i += batch) {
      await db.from("eve_arr_dupes").upsert(dupRows.slice(i, i + batch), { onConflict: "opportunity_id" })
    }
  }

  // Upsert exempt log
  const exemptRows = exemptLog.map((d) => ({ opportunity_id: d.opportunityId, data: d, updated_at: now }))
  if (exemptRows.length > 0) {
    for (let i = 0; i < exemptRows.length; i += batch) {
      await db.from("eve_arr_exempt").upsert(exemptRows.slice(i, i + batch), { onConflict: "opportunity_id" })
    }
  }
}

export async function fetchARRDeals(): Promise<{
  deals: ARRDeal[]
  dupLog: ARRDupLog[]
  exemptLog: ARRDeal[]
  ...
  const [meta, deals, dupes, exempt] = await Promise.allSettled([
    db.from("eve_arr_imports").select("import_date").eq("id", "latest").single(),
    db.from("eve_arr_deals").select("data"),
    db.from("eve_arr_dupes").select("data"),
    db.from("eve_arr_exempt").select("data"),
  ])

  const importDate =
    meta.status === "fulfilled" && meta.value.data
      ? meta.value.data.import_date
      : null

  const arrDeals: ARRDeal[] =
    deals.status === "fulfilled" && deals.value.data
      ? deals.value.data.map((r: { data: ARRDeal }) => r.data)
      : []

  const dupLog: ARRDupLog[] =
    dupes.status === "fulfilled" && dupes.value.data
      ? dupes.value.data.map((r: { data: ARRDupLog }) => r.data)
      : []

  const exemptLog: ARRDeal[] =
    exempt.status === "fulfilled" && exempt.value.data
      ? exempt.value.data.map((r: { data: ARRDeal }) => r.data)
      : []

  if (!importDate && arrDeals.length === 0) return null

  return { deals: arrDeals, dupLog, exemptLog, importDate }
}
...
export async function fetchAllRemoteState(): Promise<RemoteState> {
  ...
  if (arrResult.status === "fulfilled" && arrResult.value) {
    result.arrDeals = arrResult.value.deals
    result.arrDupLog = arrResult.value.dupLog
    result.arrExemptLog = arrResult.value.exemptLog
    result.arrImportDate = arrResult.value.importDate
  }
  if (manualArrResult.status === "fulfilled" && manualArrResult.value && manualArrResult.value.length > 0) {
    result.manualArrDeals = manualArrResult.value as unknown as ARRDeal[]
  }
  if (forecastPostsResult.status === "fulfilled" && forecastPostsResult.value) {
    result.forecastPosts = forecastPostsResult.value
  }
  if (qualificationsResult.status === "fulfilled" && qualificationsResult.value) {
    const quals: Record<string, Record<string, unknown>> = {}
    for (const row of qualificationsResult.value ?? []) {
      quals[row.opportunity_id] = row.data
    }
