...
export function parseARRReport(rawText: string): ARRImportResult {
  ...
  const idx = (names: string[], excludeSuffix?: string): number => {
    ...
    for (const name of names) {
      // Fall back to partial match — but skip columns ending in the excludeSuffix
      const partial = headers.findIndex((h) => {
        const lower = h.toLowerCase()
        if (excludeSuffix && lower.endsWith(excludeSuffix.toLowerCase())) return false
  ...
  const cols = {
    ...
    // Never match "Total ABC Currency" — must be the numeric value column
    // Fallback to "ABC Split Value" if "Total ABC" not found (combined report variant)
    totalAbc:             (() => {
      const exact = headers.findIndex((h) => h.toLowerCase() === "total abc")
      if (exact >= 0) return exact
      const noPrefix = headers.findIndex((h) => h.toLowerCase() === "abc split value")
      if (noPrefix >= 0) return noPrefix
      // Last resort: any column containing "total abc" but not "currency"
      return headers.findIndex((h) => {
        const lower = h.toLowerCase()
        return lower.includes("total abc") && !lower.endsWith("currency")
      })
    })(),
    stage:                idx(["Stage"]),
...
export function parseCombinedReport(rawText: string): CombinedReportResult {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    // ALL rows go into OI — including Closed Won (v2)
    // This ensures Won deals appear in pipeline tabs
    oiLines.push(line)

    // Closed Won ALSO goes into ARR pipeline
    if (stage === "closed won") {
      arrLines.push(line)
    }
  }
